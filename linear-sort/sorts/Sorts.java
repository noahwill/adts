...
public class Sorts {
    ...
    public static <T> void countingSort(T[] array, ToInteger<T> toInt) {
        ...
        //add loop here
        for (int i = 0; i < array.length; i++) { if (toInt.v(array[i]) > maxVal) maxVal = toInt.v(array[i]); }
        
        // The occurrences of each value (once we calculate them)
        int[] counts = new int[maxVal+1];
        // B. tabulate the occurrences of each value
        //add loop here
        for(int i = 0; i < array.length; i++) { counts[toInt.v(array[i])]++; }
        ...
        for(int i = 0; i < array.length; i++) { 
        	aux[nextPlace[toInt.v(array[i])]] = array[i];
        	nextPlace[toInt.v(array[i])]++;
        }

        // E. move them back to array
        //add loop here
        for(int i = 0; i < array.length; i++) { array[i] = aux[i]; }
    }
